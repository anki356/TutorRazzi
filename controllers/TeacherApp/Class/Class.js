...
const getClassDetails = async (req, res, next) => {
  let classDetails = {}
  classDetails = await Class.findOne({ _id: req.query.class_id,end_time:{$lte:moment().add(5,'h').add(30,'m').format("YYYY-MM-DDTHH:mm:ss")} }, { start_time: 1, end_time: 1, details: 1, grade: 1, subject: 1,notes:1, teacher_id: 1,  materials: { $slice: [0* 3, 3] }, recordings: 1,class_type:1 }).populate({
...
const getRescheduledClasses = async (req, res, next) => {
  ...
  let options = {
    ...
    select:{
      "subject":1,"name":1,"start_time":1,"end_time":1,"rescheduled_by":1,"status":1
    }
  }
  let query = {
    $and: [
...
const getUpcomingClassDetails=async(req,res)=>{
  let classDetails = {}
  classDetails = await Class.findOne({ _id: req.query.class_id,end_time:{$gte:moment().add(5,'h').add(30,'m').format("YYYY-MM-DDTHH:mm:ss")} }, { start_time: 1, end_time: 1, details: 1, grade: 1,notes:1,  teacher_id: 1,student_id:1,subject:1 ,other_information:1})
  if(classDetails===null){
    throw new Error("Incorrect Class Id")
  }
  let studentDetails=await Student.findOne({user_id:classDetails.student_id},{
...
if(classDetails.length!==0){
throw new Error("Slot Already Booked.Kindly Pick another Slot")
...
const viewRec=async(req,res)=>{
  ...
  const getTrialClassDetails = async (req, res, next) => {
    let classDetails = {}
    classDetails = await Class.findOne({ _id: req.query.class_id,class_type:"Trial" }, { start_time: 1, end_time: 1, details: 1, grade: 1, subject: 1, teacher_id: 1, notes: 1,  materials: 1, recordings: 1,response:1,reason_disliking:1,curriculum:1,status:1 ,is_rescheduled:1}).populate({
      path: 'teacher_id', select: {
       name: 1,profile_image:1
      }
    }).populate({
